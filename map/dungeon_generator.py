...
def generate_dungeon(width, height, archetype_key=None):
    """Compact dungeon generator with archetypes (functional approach)"""
    ...
    def _add_room_to_node(node):
        node["room"] = (
            node["x"] + random.randint(1, 3),
            node["y"] + random.randint(1, 3),
            max(min_room_size, random.randint(node["w"] // 3, node["w"] - 3)),
            max(min_room_size, random.randint(node["h"] // 3, node["h"] - 3)),
        )

    def split_partition(node, depth):
        if depth == 0 or node["w"] < min_room_size * 2 or node["h"] < min_room_size * 2:
            _add_room_to_node(node)
            return [node]
        ...
        if split_h:
            ...
            node["left"] = _create_node(node["x"], node["y"], split_pos, node["h"])
            node["right"] = _create_node(
                node["x"] + split_pos, node["y"], node["w"] - split_pos, node["h"]
            )
        else:
            split_pos = random.randint(min_room_size, node["h"] - min_room_size)
            node["left"] = _create_node(node["x"], node["y"], node["w"], split_pos)
            node["right"] = _create_node(
                node["x"], node["y"] + split_pos, node["w"], node["h"] - split_pos
            )

        return split_partition(node["left"], depth - 1) + split_partition(
            node["right"], depth - 1
        )
    ...
    border_wall_tile = wall_tile  
    for x in range(width):
        dungeon[0][x] = dungeon[height - 1][x] = border_wall_tile
    for y in range(height):
        dungeon[y][0] = dungeon[y][width - 1] = border_wall_tile
