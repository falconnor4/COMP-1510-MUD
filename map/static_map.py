...
def generate_new_dungeon(width=40, height=20):
    """
    Generate a new dungeon level with appropriate settings
    Returns: (player_spawn, is_new_dungeon=True)
    """
    from map.dungeon_generator import generate_dungeon_level, ARCHETYPES

    dungeon_map, spawn_x, spawn_y, archetype = generate_dungeon_level(width, height)
...
def find_tile_position(world_map, tile_value):
    """Find the position of a specific tile in the map"""
    for y in range(len(world_map)):
        for x in range(len(world_map[0])):
            if world_map[y][x] == tile_value:
                return x, y
    return None, None
...
def is_spawn_valid(x, y, current_map):
    ...
    if (
        grid_y < 0
        or grid_y >= len(current_map)
        or grid_x < 0
        or grid_x >= len(current_map[0])
    ):
        return False
