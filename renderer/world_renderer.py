...
def render_world(stdscr, player_x, player_y, player_angle, world_map, world_colors, player_state=None):
    ...
    for y in range(height // 2):
        stdscr.addstr(y, 0,
                      ceiling_chars[min(len(ceiling_chars) - 1, int(y / (height / 2) * len(ceiling_chars)))] * width,
                      curses.color_pair(9))

    for y in range(height // 2, height):
        stdscr.addstr(y, 0, floor_chars[
            min(len(floor_chars) - 1, int((y - height // 2) / (height / 2) * len(floor_chars)))] * width,
                      curses.color_pair(8))
    ...
    for column in range(0, width, resolution):
        ...
        while not hit_wall and distance_to_wall < 20:
            ...
            if not (0 <= int(test_x) < len(world_map[0]) and 0 <= int(test_y) < len(world_map)):
                hit_wall = True
                distance_to_wall = 20
            else:
                ...
                if cell_type not in [0, 4, 9]:
                    ...
                    wall_orientation = 'vertical' if abs(test_x - int(test_x)) < abs(
                        test_y - int(test_y)) else 'horizontal'
    ...
    for entity in entity_system.entities:
        ...
        bob_pixels = int(
            player_state['bob_offset'] * height / 4) if player_state and 'bob_offset' in player_state else 0
    ...
    for i, segment in enumerate(wall_segments):
        column, wall_top, wall_bottom, distance_to_wall, current_color, wall_orientation, wall_x, wall_y = \
            segment['column'], segment['top'], segment['bottom'], segment['distance'], segment['color'], segment[
                'orientation'], segment['wall_x'], segment['wall_y']

        is_left_edge = (i == 0 or abs(wall_segments[i - 1]['distance'] - distance_to_wall) > 0.5
                        or abs(wall_segments[i - 1]['wall_x'] - wall_x) > 1
                        or abs(wall_segments[i - 1]['wall_y'] - wall_y) > 1)

        is_right_edge = (i == len(wall_segments) - 1 or abs(wall_segments[i + 1]['distance'] - distance_to_wall) > 0.5
                         or abs(wall_segments[i + 1]['wall_x'] - wall_x) > 1
                         or abs(wall_segments[i + 1]['wall_y'] - wall_y) > 1)
        ...
        if wall_orientation == 'horizontal' and distance_to_wall < 20:
            shade_char = get_shading_set('horizontal')[
                min(len(get_shading_set('horizontal')) - 1, get_shading_set().find(shade_char))]
            color_attr = color_attr | curses.A_DIM
        ...
        for y in range(wall_top, wall_bottom + 1):
            try:
                ...
                char = WALL_EDGE_CHARS['top'] if y == wall_top else \
                    WALL_EDGE_CHARS['bottom'] if y == wall_bottom else \
                        WALL_EDGE_CHARS['left'] if is_left_edge and y > wall_top and y < wall_bottom else \
                            WALL_EDGE_CHARS['right'] if is_right_edge and y > wall_top and y < wall_bottom else \
                                SHADING_CHARS[min(len(SHADING_CHARS) - 1, SHADING_CHARS.find(
                                    shade_char) + 1)] if position_in_wall > 0.8 else shade_char
    ...
    for er in entity_renders:
        entity, screen_x, screen_y, entity_height, entity_scale, entity_distance = \
            er['entity'], er['screen_x'], er['screen_y'], er['height'], er['scale'], er['distance']
        ...
        if entity['type'] == entity_system.ENTITY_PROJECTILE:
            if 'pattern' in entity:
                ...
                pattern_height, pattern_width = len(pattern), len(pattern[0]) if len(pattern) > 0 else 0

                size_scale = min(1.0, 1.5 / max(1.0, entity_distance * 0.3))
                draw_height, draw_width = max(1, int(pattern_height * size_scale)), max(1,
                                                                                        int(pattern_width * size_scale))

                start_x, start_y = screen_x - draw_width // 2, screen_y - draw_height // 2

                pulse_factor = 0.7 + 0.3 * math.sin((time.time() - entity['creation_time']) * entity.get('pulse_rate',
                                                                                                         5.0) * 2 * math.pi) if entity.get(
                    'glow', False) else 1.0

                brightness = min(1.0, pulse_factor / max(0.5, entity_distance * 0.1))

                color_attr = curses.color_pair(entity['color']) | (
                    curses.A_BOLD if brightness > 0.8 else curses.A_DIM if brightness <= 0.4 else 0)
                ...
                for local_y in range(draw_height):
                    ...
                    for local_x in range(draw_width):
                        ...
                        if all(segment['column'] != screen_pos_x or segment['distance'] >= entity_distance for segment
                               in wall_segments) and pattern_x < len(pattern[pattern_y]):
                ...
                try:
                    if all(segment['column'] != screen_x or segment['distance'] >= entity_distance for segment in
                           wall_segments):
            ...
            color_attr = curses.color_pair(entity_system.ENEMY_DEAD_COLOR) if entity['state'] == 'dead' else \
                (curses.color_pair(entity_system.ENEMY_ALERT_COLOR) | curses.A_BOLD if entity[
                                                                                           'state'] == 'attack' and int(
                    time.time() * 4) % 2 == 0 else
                 curses.color_pair(entity_system.ENEMY_ALERT_COLOR) | curses.A_BOLD if entity['state'] == 'attack' else
                 curses.color_pair(entity_system.ENEMY_ALERT_COLOR) if entity['state'] == 'chase' else
                 curses.color_pair(entity['color']))
            ...
            for i in range(scaled_height):
                ...
                for j in range(scaled_width):
                    ...
                    if all(segment['column'] != screen_pos_x or segment['distance'] >= entity_distance for segment in
                           wall_segments) and j < len(line):
                        try:
                            char = line[j]
                            stdscr.addch(screen_pos_y, screen_pos_x, char, color_attr)
                        except curses.error:
